...
import { Alternative, Notebook, Question, RightAlternative, SubTopic, Topic } from "@prisma/client"
import Select, { GroupedOptionsType, GroupType, OptionsType } from 'react-select'
...
const mapQuestionWithAllToTypeToPost = ({notebook, subTopic, ...newObj}: QuestionWithAll) => {
...
const EditQuestion: React.FC<{
    ...
    const postQuestion = async (_question: Question & {
        alternatives: Alternative[];
        rightAlternative: RightAlternative;
    }) => {
    ...
    const topic: any = topics.find(x => x?.options.some(y => Number(y.value) == state.subTopicId))?.options?.find(y => Number(y.value) == state.subTopicId)
                                        ...
                                        <div className="grid grid-cols-3 gap-4">
                                            ...
                                            <div className="col-span-6 sm:col-span-1 ">
                                                ...
                                                <div className="mt-1 relative rounded-md shadow-sm">
                                                    ...
                                                    placeholder="0.00"                        
                                                    onChange={x => dispatch({type: ActionType.UPDATE_SOLUTION, solution: x.target.value})}
                                            ...
                                            <div className="col-span-6 sm:col-span-3">
                                                ...
                                                <textarea className="resize-y mt-1 font-mono focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" onChange={x => dispatch({type: ActionType.UPDATE_DESCRIPTION, description: x.target.value}) } value={state?.question ?? ""}/>
                                            </div>
