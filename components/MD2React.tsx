...
export const MD2React: React.FC<{
    ...
    .use(inspectUrls, {
        inspectEach({ url, node }) {
            if(new RegExp("^(?!www\.|(?:http|ftp)s?://|[A-Za-z]:\\|//).*").test(url)){
                node.properties.src = absolute(props.filePath, url)
            }
        },  
        selectors: [
            "img[src]"
    ...
    .use(rehype2react, { 
        ...
        components: {
            ...
            form: (props: any) => <form {...props} action="/api/hello" target="_blank">{props.children}</form>,
